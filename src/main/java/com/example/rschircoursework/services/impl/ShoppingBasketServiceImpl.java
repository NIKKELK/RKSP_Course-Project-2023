    ...
    public int getTotalPrice(List<ShoppingBasket> purchases) {
        ...
        for (ShoppingBasket purchase : purchases) {
            result += (iItemService.findById(purchase.getItemId())).getCost()
                    * (1 - 0.01 * iItemService.findById(purchase.getItemId()).getDiscount().getSale())
                    * purchase.getAmount();
        }
        return result;
    }
        ...
        for (int i = 0; i < userProducts.size(); i++) {
            result +=
                    (i + 1) + ") " +
                            ...
                            + " р.)" +
                            ", Цена по скидке: " +
                            (userProducts.get(i).getCost() * 0.01 * (100 - userProducts.get(i).getDiscount().getSale()) * userPurchases.get(i).getAmount())
                            + " р.)" +
                            "<br>";
        }
