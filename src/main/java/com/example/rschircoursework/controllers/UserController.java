...
public class UserController extends AbstractController<User, IUserService> {
    ...
    @Autowired
    protected UserController(IUserService service, IPhoneBrandService iPhoneBrandService,
    ...
    public String products(Authentication authentication,
        ...
        Long phoneId = (IpetId == null) ? 0 : IpetId;
        ...
        model.addAttribute("pId", phoneId);
        model.addAttribute("pTId", productTypeId);
        model.addAttribute("types", iItemTypeService.getAll());

        if (phoneId == 0 && productTypeId == 0) {
            model.addAttribute("items", iItemService.getAllSorting(sortId, iItemService.getAll()));
        }
        else if (phoneId == 0 && productTypeId != 0) {
            model.addAttribute("items",iItemService.getAllSorting(sortId, iItemService.getItemByItemType(productTypeId)));
        }
        else if (phoneId != 0 && productTypeId == 0) {
            model.addAttribute("items", iItemService.getAllSorting(sortId,iItemService.getItemByPhoneBrandId(phoneId)));
        } else {
            model.addAttribute("items", iItemService.getAllSorting(sortId,iItemService.getItemByPhoneBrandIdAndTypeId(phoneId, productTypeId)));
        }
